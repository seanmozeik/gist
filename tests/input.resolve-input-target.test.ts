...
describe('resolveInputTarget', () => {
  ...
  it('unescapes common pasted backslash escapes for query separators', () => {
    expect(resolveInputTarget(String.raw`https://www.youtube.com/watch\?v\=497Ov6kV4KM`)).toEqual({
      kind: 'url',
  ...
  it('extracts the last URL from pasted text and normalizes it', () => {
    expect(
      resolveInputTarget(
        String.raw`https://www.youtube.com/watch\?v\=497Ov6kV4KM (https://www.youtube.com/watch%5C?v%5C=497Ov6kV4KM)`,
      ),
    ).toEqual({ kind: 'url', url: 'https://www.youtube.com/watch?v=497Ov6kV4KM' });
  });
  ...
  it('resolves - to stdin input', () => {
    expect(resolveInputTarget('-')).toEqual({ kind: 'stdin' });
  });
