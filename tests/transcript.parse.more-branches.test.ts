...
describe('transcript parsing branches', () => {
  it('skips note/style blocks and ignores cues without valid starts', () => {
    ...
    expect(vttToSegments(vtt)).toEqual([{ endMs: null, startMs: 2000, text: 'Hello world' }]);
  });
  ...
  it('parses json segments from startMs/endMs and trims text', () => {
    expect(
      jsonTranscriptToSegments([
        { endMs: 2300, startMs: 1200, text: '  Hello   there  ' },
        { endMs: 2400, startMs: 'bad', utf8: 'world' },
        null,
      ]),
    ).toEqual([{ endMs: 2300, startMs: 1200, text: 'Hello there' }]);
  });
