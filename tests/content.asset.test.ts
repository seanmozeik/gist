...
describe('asset helpers', () => {
  it('resolves input targets from files and urls', () => {
    ...
    expect(resolveInputTarget(filePath)).toEqual({ filePath, kind: 'file' });
  ...
  it('handles file urls and rejects unsupported protocols', () => {
    ...
    expect(resolveInputTarget(fileUrl.toString())).toEqual({ filePath, kind: 'file' });
  ...
  it('classifies urls as assets or websites', async () => {
    await expect(
      classifyUrl({ fetchImpl: fetch, timeoutMs: 10, url: 'https://example.com/image.jpg' }),
    ).resolves.toEqual({ kind: 'asset' });

    await expect(
      classifyUrl({ fetchImpl: fetch, timeoutMs: 10, url: 'https://example.com/page.html' }),
