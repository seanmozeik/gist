...
describe('config env', () => {
  ...
  it('merges config env as fallback (existing env wins)', () => {
    const merged = mergeConfigEnv({
      config: { env: { CUSTOM_FLAG: 'enabled', OPENAI_API_KEY: 'sk-config' } },
      env: { OPENAI_API_KEY: 'sk-shell', OTHER: '1' },
    });
  ...
  it('uses config env when shell value is blank', () => {
    const merged = mergeConfigEnv({
      config: { env: { OPENAI_API_KEY: 'sk-config' } },
      env: { OPENAI_API_KEY: '   ' },
    });
  ...
  it('prefers explicit env map over legacy apiKeys', () => {
    const merged = mergeConfigEnv({
      config: { apiKeys: { openai: 'legacy-openai' }, env: { OPENAI_API_KEY: 'explicit-openai' } },
      env: {},
    });
