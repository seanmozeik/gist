...
describe('chrome/daemon-recovery', () => {
  it('retries once after daemon recovers', () => {
    ...
    expect(recovery.maybeRecover({ currentUrlMatches: true, isIdle: true, isReady: false })).toBe(
      false,
    );
    expect(recovery.getPendingUrl()).toBe('https://example.com');

    expect(recovery.maybeRecover({ currentUrlMatches: true, isIdle: true, isReady: true })).toBe(
      true,
    );
  ...
  it('does not recover while inflight', () => {
    ...
    expect(recovery.maybeRecover({ currentUrlMatches: true, isIdle: false, isReady: true })).toBe(
      false,
    );
    expect(recovery.getPendingUrl()).toBe('https://example.com');
  });
  ...
  it('clears pending when URL changes', () => {
    ...
    expect(recovery.maybeRecover({ currentUrlMatches: false, isIdle: true, isReady: true })).toBe(
      false,
    );
    expect(recovery.getPendingUrl()).toBeNull();
  });
