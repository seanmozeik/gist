...
describe('daemon/server listen host', () => {
  ...
  it('binds to all interfaces in Windows container mode', () => {
    ...
    expect(
      resolveDaemonListenHost({ CONTAINER_SANDBOX_MOUNT_POINT: 'C:\\ContainerMappedDirectories' }),
    ).toBe('0.0.0.0');
  });

  it('keeps loopback on non-Windows hosts even with container markers', () => {
    Object.defineProperty(process, 'platform', { value: 'linux' });
    expect(resolveDaemonListenHost({ RUNNING_IN_CONTAINER: '1' })).toBe('127.0.0.1');
  });
});
