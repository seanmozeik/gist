...
describe('createTempFileFromStdin', () => {
  it('writes UTF-8 text stdin to a .txt temp file', async () => {
    const temp = await createTempFileFromStdin({ stream: streamFrom('hello from stdin\n') });
    try {
  ...
  it('rejects whitespace-only text stdin', async () => {
    await expect(createTempFileFromStdin({ stream: streamFrom('   \n\t  ') })).rejects.toThrow(
      'Stdin is empty',
    );
  });
  ...
  it('detects binary stdin and preserves bytes', async () => {
    ...
    const temp = await createTempFileFromStdin({ stream: streamFrom(pngBytes) });
    try {
