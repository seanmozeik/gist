...
function parseCliProviderList(
  ...
  if (!Array.isArray(raw)) {
    throw new TypeError(`Invalid config file ${path}: "${label}" must be an array.`);
  }
...
export function parseEnvConfig(root: Record<string, unknown>, path: string): EnvConfig | undefined {
  ...
  for (const [rawKey, rawValue] of Object.entries(value)) {
    ...
    if (typeof rawValue !== 'string') {
      throw new TypeError(`Invalid config file ${path}: "env.${rawKey}" must be a string.`);
    }
    env[key] = rawValue;
  }
