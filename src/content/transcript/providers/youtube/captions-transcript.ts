...
export const fetchTranscriptFromCaptionTracks = async (
  ...
  if (!bootstrap) {
    return fetchTranscriptViaAndroidPlayer(fetchImpl, { html, skipAutoGenerated, videoId });
  }
  ...
  const clientContext = isObjectLike(contextRecord.client) ? contextRecord.client : {};
  const requestBody: Record<string, unknown> = {
    contentCheckOk: true,
    context: { ...contextRecord, client: { ...clientContext, originalUrl } },
  ...
  try {
    ...
    const response = await fetchWithTimeout(
      fetchImpl,
      `https://www.youtube.com/youtubei/v1/player?key=${apiKey}`,
      { body: JSON.stringify(requestBody), headers, method: 'POST' },
    );

    if (!response.ok) {
      return await fetchTranscriptViaAndroidPlayer(fetchImpl, { html, skipAutoGenerated, videoId });
    }
...
const extractTranscriptFromPlayerPayload = async (
  ...
  return findFirstTranscript(fetchImpl, tracksToUse, 0);
};
...
const downloadCaptionTrack = async (
  ...
  try {
    const response = await fetchWithTimeout(fetchImpl, json3Url, { headers: REQUEST_HEADERS });
    if (!response.ok) {
    ...
    return downloadXmlTranscript(fetchImpl, baseUrl);
  }
};
...
const parseJsonTranscript = (raw: string): TranscriptPayload | null => {
  try {
    ...
    return { segments: segments.length > 0 ? segments : null, text: transcript };
  } catch {
    return null;
  }
};
...
const parseXmlTranscript = (xml: string): TranscriptPayload | null => {
  ...
  while (match) {
    ...
    if (decoded.length > 0) {
      lines.push(decoded);
      const tag = match[0] ?? '';
      const startMatch = /\bstart\s*=\s*(['"])([^'"]+)\1/i.exec(tag);
      const durMatch = /\bdur\s*=\s*(['"])([^'"]+)\1/i.exec(tag);
      ...
      if (startMs != null) {
        segments.push({
          endMs: durationMs != null ? startMs + durationMs : null,
          startMs,
          text: decoded.replaceAll(/\s+/g, ' ').trim(),
        });
      }
    }
