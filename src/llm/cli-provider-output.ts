    ...
    try {
      ...
      for (const candidate of candidates) {
        ...
        if (input !== null || outputTokens !== null || totalTokens !== null) {
          usage = { completionTokens: outputTokens, promptTokens: input, totalTokens };
        }
      }
      if (costUsd === null) {
        const costValue =
...
export function parseCodexOutputFromJsonl(output: string): {
  ...
  const trimmed = output.trim();
  if (!trimmed) {
    return { sawStructuredEvent: false, text: null };
  }
...
export function parseOpenCodeOutputFromJsonl(output: string): {
  ...
  return { costUsd: null, text: trimmed, usage: null };
}
