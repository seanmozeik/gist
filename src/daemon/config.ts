...
export interface DaemonConfigV2 {
  version: 2;
  token: string;
  tokens: string[];
  port: number;
...
export function daemonConfigTokens(config: DaemonConfig): string[] {
  if (config.version === 2) {
    const tokens = [...new Set(config.tokens.filter((token): token is string => typeof token === 'string').map((token) => trimDaemonToken(token)))];
...
export async function readDaemonConfig({
  ...
  try {
    ...
    throw new Error(`Invalid daemon config JSON at ${configPath}: ${message}`, { cause: error });
  }
  ...
  if (version === 1) {
    ...
    return { env: envSnapshot, installedAt, port, token, tokens: [token], version: 2 };
  }
