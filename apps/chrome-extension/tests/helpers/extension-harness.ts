...
const defaultUiState: UiState = {
  ...
  status: '',
  tab: { id: null, title: null, url: null },
};
...
export async function sendPanelMessage(page: Page, message: object) {
  ...
  const portName = await page.evaluate(() => {
    const port = (
      window as { __summarizePanelPort?: { name?: string; postMessage?: (value: object) => void } }
    ).__summarizePanelPort;
    return typeof port?.name === 'string' ? port.name : null;
  });
  const windowId =
    portName?.startsWith('sidepanel:')
      ? Number.parseInt(portName.slice('sidepanel:'.length), 10)
      : Number.NaN;
  const background =
    page.context().serviceWorkers()[0] ??
    (await page.context().waitForEvent('serviceworker', { timeout: 5000 }));
  ...
  await page.evaluate((payload) => {
    const port = (window as { __summarizePanelPort?: { postMessage: (value: object) => void } })
      .__summarizePanelPort;
  ...
  const result = await Promise.race([
    background.evaluate(
      async ({ scriptFile, prefix }) => {
        ...
        await chrome.scripting.executeScript({ files: [scriptFile], target: { tabId: target.id } });
        return { ok: true };
      },
      { prefix: urlPrefix ?? '', scriptFile: file },
    ),
    new Promise<{ ok: false; error: string }>((resolve) =>
      setTimeout(() =>{  resolve({ error: 'inject timeout', ok: false }); }, 5000),
    ),
  ]);
  ...
  await expect
    .poll(async () => {
      return await background.evaluate(
        ...
        { limit: maxChars, prefix: urlPrefix },
      );
    })
    .toBe(true);
}
...
export async function openExtensionPage(
  ...
  await page.goto(getExtensionUrl(harness, pathname), { waitUntil: 'domcontentloaded' });
    ...
    await page.waitForFunction(
      () => {
        ...
        return (
          ...
          global.__summarizeTestHooks.getSettingsHydrated()
        );
      },
      null,
      { timeout: 10_000 },
