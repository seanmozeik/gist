...
export function createSidepanelBgMessageRuntime(options: {
  ...
  getSlidesSummaryState: () => { complete: boolean; markdown: string };
  updateSlideSummaryFromMarkdown: (
    markdown: string,
    opts?: { preserveIfEmpty?: boolean; source?: 'slides' | 'summary' },
  ) => void;
  renderInlineSlidesFallback: () => void;
  schedulePanelCacheSync: () => void;
  consumeUiCache: (
    msg: UiCacheMessage,
  ) => { tabId: number; url: string; cache: unknown; preserveChat: boolean } | null;
      ...
      handleSidepanelBgMessage({
        ...
        handleSlidesRun: (slidesRun: SlidesRunMessage) => {
          ...
          if (
            ...
            options.rememberPendingSlidesRun({ runId: slidesRun.runId, url: targetUrl });
            return;
          }
