...
export function splitSlidesMarkdown(markdown: string): { summary: string; slides: string | null } {
  ...
  return { slides: slides.length > 0 ? slides : null, summary };
}
...
export function selectMarkdownForLayout({
  ...
  if (!trimmed) {return '';}
  if (shouldHideSummaryForSlides({ hasSlides, inputMode, slidesEnabled })) {
    return '';
  }
...
export function normalizeOcrText(raw: string | null | undefined): string {
  ...
  const wordishTokens = tokens
    .map((token) => token.replaceAll(/[^\p{L}]/gu, ''))
  ...
  const longWordishTokens = tokens.filter((token) => {
    const stripped = token.replaceAll(/[^\p{L}\p{N}]/gu, '');
...
export function deriveSlideSummaries({
  ...
  const hasMeaningfulSlides = [...parsed.values()].some((text) => text.trim().length > 0);
  ...
  for (const [index, text] of parsed) {
    const parsedSlide = splitSlideTitleFromText({ slideIndex: index, text, total });
...
export function resolveSlidesTextState({
  ...
  return { slidesOcrAvailable, slidesTextMode, slidesTextToggleVisible: allowOcr };
}
