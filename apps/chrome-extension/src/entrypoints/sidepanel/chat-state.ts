...
export interface ChatHistoryLimits { maxMessages: number; maxChars: number }

export interface ChatContextUsage { totalChars: number; percent: number; totalMessages: number }
...
export function computeChatContextUsage(
  ...
  return { percent, totalChars, totalMessages: messages.length };
}
