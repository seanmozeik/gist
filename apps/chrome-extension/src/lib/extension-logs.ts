...
export const readExtensionLogs = async (tail: number): Promise<ExtensionLogResult> => {
  const store = getStorage();
  if (!store) {
    return { lines: [], mtimeMs: null, ok: false, sizeBytes: 0, truncated: false };
  }
  const res = await store.get(LOG_KEY);
  ...
  const mtimeMs = parseLogMtime(allLines.at(-1) ?? null);
  const sizeBytes = allLines.reduce((sum, line) => sum + line.length, 0);
  return { lines, mtimeMs, ok: true, sizeBytes, truncated: total > lines.length };
};
