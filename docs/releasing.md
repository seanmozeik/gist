...
- Tag + GitHub release.
- Verify the Homebrew/core formula so `brew install gist` matches the latest tag.
...
3. Verify the Homebrew/core formula reflects the new version:
   - `brew install gist`
   - `gist --version` matches tag.
   - Run a feature added in the release (for example `gist daemon install`).
